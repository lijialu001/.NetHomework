...
namespace _3._1
{
	class ConsoleInterface
	{
		...
		public static void AddOrder(OrderService os)
		{
			...
			if (str == "") return;
			Order order = new Order { Customer = str };
			AddItem(order);
		...
		private static void AddItem(Order order)
		{
			...
			while (loop)
			{
				...
				detail.LoadName = str;
				detail.Price = 0;
				bool duplicated = (from OrderDetails i in order.GetDetails() where i.LoadName == str select i).Count() != 0;
				if (duplicated)
				{
		...
		public static void UpdateItem(Order order)
		{
			...
			if (index == -1) return;
			OrderDetails detail = details[index - 1];
			string oldName = detail.LoadName;
			Console.WriteLine("请输入新的商品名称，留空则保留 [" + detail.LoadName + "]：");
			string newName = Console.ReadLine();
			if (newName != "") detail.LoadName = newName;
			...
			Console.WriteLine("请输入新的商品数量，留空则保留 [" + $"{detail.Number:F3}" + "]：");
			decimal? count = ReadDecimal(true);
			if (count.HasValue) detail.Number = count.Value;
			order.UpdateItem(detail, oldName);
		}
		...
		public static void DeleteItem(Order order)
		{
			...
			int index = ReadInt(1, details.Count, true) ?? -1;
			if (index == -1) return;
			Console.WriteLine("确定要删除商品 " + details[index - 1].LoadName + " 吗？Y/N");
			bool confirm = ReadBool();
			if (confirm) order.RemoveItem(details[index - 1].LoadName);
		}
		...
		public static bool DeleteOrder(OrderService os, Order order)
		{
			...
			bool confirm = ReadBool();
			if (confirm) os.DeleteOrder(order.OrderId);
			return confirm;
		}

		public static void ShowOrderDetails(Order order)
		{
			Console.WriteLine("客户名：" + order.Customer);
